...
func HttpProxyFileUpload(file *multipart.FileHeader, fileKey string, addFields map[string]string,
	...
	if err != nil {
		slog.Error("Upload Create form file failed", slog.Any("错误原文", err))
		return
	}
	...
	if err != nil {
		slog.Error("Write to form file failed", slog.Any("错误原文", err))
		return
	}
	for fieldKey, fieldVal := range addFields {
		if err = writer.WriteField(fieldKey, fieldVal); err != nil {
			slog.Error("WriteField failed", slog.Any("错误原文", err))
			return
		}
	}
	// 发送表单
	...
	if err != nil {
		slog.Error("Post failed", slog.Any("错误原文", err))
